...
from multi-neuron-experiments.synaptic_model import N_syn
from multi-neuron-experiments.helpers import detect_spike_times
...
# Neurons 0,1,2,3,4,5,6 feed into neuron 7
connections = [(i, N-1) for i in range(N-1)]

initial_state = [(V_rest, m_inf(V_rest), h_inf(V_rest), n_inf(V_rest)) for _ in range(N)]

for uA in np.arange(0.0, 30.0, 1.0):

    I_ext_functions = [lambda t, amp=uA: amp for _ in range(N-1)] + [lambda t: 0.0]
